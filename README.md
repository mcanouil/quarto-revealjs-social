...
```bash
quarto add mcanouil/quarto-revealjs-social@0.1.0
```
...
The extension reads standard Quarto social metadata first and falls back to scoped `extensions.revealjs-social` options.
...
| Emitted tag                                | Standard source(s)                                                       | Scoped fallback (`extensions.revealjs-social.*`) |
| ------------------------------------------ | ------------------------------------------------------------------------ | ------------------------------------------------ |
...
> [!NOTE]
> An image referenced only through metadata is not seen by Quarto's resource scanner and would not be copied on publish.
> When the image is a local path, the filter appends a hidden `Image` element so Quarto detects and copies it.
...
- [RevealJS](https://m.canouil.dev/quarto-revealjs-social/)
