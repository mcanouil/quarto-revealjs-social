...
`social/` is a copy of the extension this repository publishes, taken from `_extensions/` at the repository root.
...
A shortcode contributed by an extension that is only linked, or only copied in by `pre-render`, is reported as `Shortcode 'social' not found` and renders as nothing.
