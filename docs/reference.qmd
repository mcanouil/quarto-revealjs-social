...
These counts are for a deck whose image exists.
Quarto reads the file to write `og:image:width` and `og:image:height`, so a deck pointing at a missing image gets fewer tags from Quarto and more that only the filter writes.
...
Quarto and the filter each write their own tags, and the head interleaves them, so the order of two copies of one property is not fixed.
A scraper reads the first it meets, so do not rely on either source winning.
