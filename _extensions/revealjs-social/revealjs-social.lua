...
--- @module "revealjs-social"
--- @license MIT
--- @copyright 2026 Mickaël Canouil
--- @author Mickaël Canouil
--- @brief Pandoc AST filter for revealjs-social.
...
--- back to scoped extensions.revealjs-social options.

--- Extension name constant
local EXTENSION_NAME = 'revealjs-social'
...
--- scoped extensions.revealjs-social option.
--- @param meta table The document metadata table
--- @param standard_paths table<integer, table<integer, string>> Ordered list of key paths to try first
--- @param scoped_key string|nil The extensions.revealjs-social key used as fallback
...
--- Append a hidden Image block so Quarto's resource scanner detects and copies
--- a locally referenced social-card image on publish.
--- @param doc pandoc.Pandoc The document being processed
--- @param image string The local image path
--- @return nil
local function force_resource_discovery(doc, image)
  local hidden = pandoc.Div(
    pandoc.Plain(pandoc.Image({}, image, '', pandoc.Attr('', {}, { { 'fig-alt', '' } }))),
    pandoc.Attr('', {}, { { 'style', 'display: none;' }, { 'aria-hidden', 'true' } })
  )
  doc.blocks:insert(hidden)
end
...
local function process_document(doc)
  ...
  if image and not is_absolute_url(image) then
    force_resource_discovery(doc, image)
  end
