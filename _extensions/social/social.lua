...
local function process_document(doc)
  ...
  local image_alt = resolve(meta, { { 'open-graph', 'image-alt' }, { 'image-alt' } }, 'image-alt')
  ...
  -- Open Graph writes a locale as language_TERRITORY, while `lang` is a BCP 47
  -- tag using a hyphen, so a value taken from there needs converting.
  add_meta('property', 'og:locale', locale and locale:gsub('-', '_') or nil)
